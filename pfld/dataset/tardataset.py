...
class TarDataset(Dataset):
    ...
    Args:
      ...
      extensions (tuple): Extensions (strings starting with a dot), only files with
        these extensions will be iterated. Default: png/jpg/jpeg.
      is_valid_file (callable): Optional function that takes file information as input
        (tarfile.TarInfo) and outputs True for files that need to be iterated; overrides
        extensions argument.
    ...
    def __init__(
        ...
        # print(self.get_text_file("WLFW_data/train_data/list.txt"))
    ...
    def filter_samples(self, is_valid_file=None, extensions=(".png", ".jpg", ".jpeg")):
        ...
        Args:
          ...
          is_valid_file (callable): Optional function that takes file information as
            input (tarfile.TarInfo) and outputs True for files that need to be iterated;
            overrides extensions argument.
            Example: lambda m: m.isfile() and m.name.endswith('.png')
        """
    ...
    def __getitem__(self, index):
        ...
        By default, this simply applies the given transforms or converts the image to a
        tensor if none are specified.
